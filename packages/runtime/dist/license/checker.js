...
exports.TIER_MINIMUM_FEATURES = exports.TIER_RANK = exports.TransparentGuardError = void 0;
exports.verifyOfflineKey = verifyOfflineKey;
exports.checkLicense = checkLicense;
exports.assertFeature = assertFeature;
exports.assertTier = assertTier;
...
class TransparentGuardError extends Error {
    code;
    detail;
...
exports.TransparentGuardError = TransparentGuardError;
// ---------------------------------------------------------------------------
// Runtime payload validation                                      (fix #3)
// ---------------------------------------------------------------------------
function validateOfflinePayload(raw) {
    const fail = (msg) => {
        throw new TransparentGuardError(`Offline license key payload is invalid: ${msg}`, "invalid_key");
    };
    ...
    if (typeof p["tier"] !== "string" || !VALID_TIERS.has(p["tier"]))
    ...
    if (typeof p["cid"] !== "string" || !p["cid"].trim())
    ...
    if (p["env"] !== undefined &&
        (typeof p["env"] !== "string" || !p["env"].trim()))
 ...
 */
function verifyOfflineKey(licenseKey) {
    if (!licenseKey.startsWith("tgk1_")) {
        throw new TransparentGuardError("Invalid offline license key format — expected prefix tgk1_. " +
            "Generate a key with `tg keys create` or check transparentguard.dev.", "invalid_key");
    }
    const without = licenseKey.slice("tgk1_".length);
    const dotIdx = without.lastIndexOf(".");
    if (dotIdx === -1) {
        throw new TransparentGuardError("Invalid offline license key — missing signature component.", "invalid_key");
    }
    ...
    if (!sigValid) {
        throw new TransparentGuardError("Offline license key signature verification failed. " +
            "The key may be corrupted or was not issued by TransparentGuard.", "invalid_key");
    }
    // Decode payload
    let rawPayload;
    try {
        ...
        throw new TransparentGuardError("Offline license key payload could not be decoded.", "invalid_key");
    }
    ...
    if (payload.iat > nowSec + CLOCK_SKEW_SEC) {
        throw new TransparentGuardError(`Offline license key has a future issue time ` +
            `(iat: ${new Date(payload.iat * 1000).toISOString()}). ` +
            "Verify your system clock is correct.", "invalid_key");
    }
    // Fix #5 — reject keys older than 2 years regardless of exp
    if (nowSec - payload.iat > MAX_KEY_AGE_SEC) {
        throw new TransparentGuardError(`Offline license key is too old ` +
            `(issued ${new Date(payload.iat * 1000).toISOString()}). ` +
            "Generate a fresh key with `tg keys create`.", "invalid_key");
    }
    // Standard expiry
    if (payload.exp < nowSec) {
        throw new TransparentGuardError(`Offline license key expired at ${new Date(payload.exp * 1000).toISOString()}. ` +
            "Generate a new key with `tg keys create` or upgrade at transparentguard.dev.", "trial_expired");
    }
    ...
    if (payload.env && expectedEnv && payload.env !== expectedEnv) {
        throw new TransparentGuardError(`Offline license key is scoped to environment "${payload.env}" ` +
            `but this runtime has TG_LICENSE_ENV="${expectedEnv}". ` +
            "Use the correct key for this environment.", "invalid_key");
    }
    return {
        valid: true,
        tier: payload.tier,
...
function setCached(apiKey, status) {
    evictOldest(cache);
...
function getGraceCached(apiKey) {
...
function setGraceCached(apiKey, status) {
...
function warnServerlessOnce() {
    if (_serverlessWarned || !isServerlessEnvironment())
        return;
    _serverlessWarned = true;
    console.warn("[TransparentGuard] Serverless environment detected. " +
...
async function checkApiKey(apiKey, apiBaseUrl) {
    ...
    try {
        ...
        if (response.status === 401 || response.status === 403) {
            ...
            if (code === "trial_expired") {
                throw new TransparentGuardError("Your TransparentGuard trial has ended. Upgrade at transparentguard.dev to continue.", "trial_expired");
            }
            throw new TransparentGuardError("Invalid TransparentGuard API key. Check your key at transparentguard.dev.", "invalid_key");
        }
        if (response.status === 429) {
            throw new TransparentGuardError("TransparentGuard license check rate limited. Retrying with cached status.", "rate_limited");
        }
        if (!response.ok) {
            throw new TransparentGuardError(`TransparentGuard API returned ${response.status}.`, "api_unreachable");
        }
        ...
        if (err instanceof TransparentGuardError)
            throw err;
        throw new TransparentGuardError(`Cannot reach TransparentGuard API: ${String(err)}`, "api_unreachable");
    }
    finally {
 ...
 */
async function checkLicense(apiKey, apiBaseUrl = DEFAULT_API_BASE, offlineMode = false) {
    ...
    if (offlineMode) {
        console.warn("[TransparentGuard] The `offlineMode` parameter is deprecated and will be removed in v0.5.0. " +
    ...
    try {
        ...
        if (err instanceof TransparentGuardError) {
            if (err.code === "api_unreachable" || err.code === "rate_limited") {
                ...
                if (graceStatus) {
                    console.warn(`[TransparentGuard] ${err.message} Using cached license status (grace window active).`);
                    return graceStatus;
                }
                // No grace status — hard fail. Give extra context in serverless.
                throw new TransparentGuardError("TransparentGuard license server unreachable and no cached status is available. " +
                    (isServerlessEnvironment()
                        ? "In serverless environments the grace cache resets on cold starts — " +
                            "consider using TG_LICENSE_KEY for reliable operation. "
                        : "") +
                    `Verify network connectivity to api.transparentguard.dev. Original error: ${err.message}`, "api_unreachable");
            }
            throw err;
        }
        throw err;
 ...
 */
function assertFeature(status, feature, featureDescription) {
    if (!status.features.includes(feature)) {
        throw new TransparentGuardError(`${featureDescription} requires a paid TransparentGuard plan. Upgrade at transparentguard.dev.`, "feature_requires_paid_tier", feature);
    }
}
/**
 ...
 * Throws TransparentGuardError with code "feature_requires_paid_tier" if not.
 */
function assertTier(status, required, featureDescription) {
    if (exports.TIER_RANK[status.tier] < exports.TIER_RANK[required]) {
        throw new TransparentGuardError(`${featureDescription} requires a ${required} plan or above. Upgrade at transparentguard.dev.`, "feature_requires_paid_tier", required);
    }
}
