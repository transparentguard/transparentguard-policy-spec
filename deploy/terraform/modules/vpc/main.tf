...
resource "aws_security_group" "proxy" {
  ...
  description = "Security group for the proxy ECS tasks — allows inbound only from ALB"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "rds" {
  ...
  description = "Security group for RDS — allows PostgreSQL only from proxy tasks"
  vpc_id      = aws_vpc.main.id

  ingress {
    description             = "PostgreSQL from proxy tasks only"
    from_port               = 5432
    to_port                 = 5432
    protocol                = "tcp"
